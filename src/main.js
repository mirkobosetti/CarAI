...
const traffic = [
	...
	new Car(road.getLaneCenter(0), -700, 30, 50, 'DUMMY'),
	new Car(road.getLaneCenter(1), -700, 30, 50, 'DUMMY'),
	new Car(road.getLaneCenter(0), -900, 30, 50, 'DUMMY'),
	new Car(road.getLaneCenter(0), -900, 30, 50, 'DUMMY'),
]
let bestCar = cars[0]
...
function animate(time) {
	...
	for (let i = 0; i < traffic.length; i++) {
		...
		if (traffic[i].y > bestCar.y + 300) {
			traffic[i].y -= 1000
			traffic[i].x = road.getLaneCenter(Math.floor(Math.random() * 3))
		}
